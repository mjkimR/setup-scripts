...
3. **Write the document** (structure below) and show the user its absolute
   path plus a 3–5 line summary of what it captures.
4. **Execute** per the chosen mode (`auto` → headless immediately with the
   ...
   Headless is synchronous; make no other tool calls while it runs. The
   ...
   2 it stopped part-way (partial work may exist; check before retrying).
...
5. **Close the loop.** The prompt tells the receiver to write a completion
   ...
   document itself stays untouched. Headless: relay the receiver's closing
...
there as `<YYYY-MM-DD>-<slug>.md`, taking the date from `date +%Y-%m-%d`.
